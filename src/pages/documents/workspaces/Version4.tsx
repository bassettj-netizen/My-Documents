...
/**
 * Workspaces — Version 4: "Chat-first with a documents drawer".
 * The space detail view leads with a task/chat composer — documents live in a
 * slide-over drawer reached via the doc-count pill, not a tab or split pane.
 */
export default function WorkspacesVersion4() {
  const {
  ...
  const goToLanding = () => { setDetailView('landing'); setDrawer(null); setPreviewDocId(null) }
      ...
      <div ref={detailRef} style={{ height: detailHeight ?? '100%', overflow: 'hidden', position: 'relative', backgroundColor: colorPalette.white }}>
        ...
        {detailView === 'docPreview' && previewDoc && (
          <DocPreviewView
            ...
            onBack={() => { setDetailView('landing'); setDrawer('documents') }}
        ...
        {detailView === 'documentsTable' && (
          <DocumentsTableView
            ...
            page={docsPage}
            setPage={setDocsPage}
            onCollapse={() => { setDetailView('landing'); setDrawer('documents') }}
            onOpenFilter={() => setDrawer('filter')}
            onOpenAdd={() => setDrawer('add')}
            onOpenDoc={id => { setPreviewDocId(id); setDetailView('docPreview') }}
        ...
        <DocumentsDrawer
          ...
          onOpenFilter={() => setDrawer('filter')}
          onOpenAdd={() => setDrawer('add')}
          onOpenDoc={id => { setPreviewDocId(id); setDetailView('docPreview'); setDrawer(null) }}
          onSendToChat={sendToChat}
          onDeleteDocs={ids => setSpaceDocs(selectedSpace.id, docs.filter(d => !ids.has(d._id)))}
          onClearFilter={(category, value) => {
            if (category === 'type') setTypeFilter(prev => prev.filter(v => v !== value))
...
function MessageComposer({ value, onChange, onSend, placeholder, autoSize, seedAttachments, onSeedAttachmentsConsumed }: {
  ...
  return (
    ...
    >
      {hasAttachments && (
        <div ref={chipRowRef} style={{ position: 'absolute', top: spacing(2), left: spacing(2), right: spacing(2), display: 'flex', flexWrap: 'wrap', gap: spacing(2), zIndex: 2 }}>
          {attachments.map(a => (
      ...
      <div style={{ position: 'absolute', left: spacing(2), bottom: spacing(2), display: 'flex', gap: spacing(2) }}>
        ...
        >
          <ButtonTertiary leftIcon={iconType.SparksOutlined}>Prompt templates</ButtonTertiary>
        </Dropdown>
      </div>
      <input
        ref={fileInputRef}
...
function DocCard({ space, doc, source, selected, isNew, onToggle, onOpen, onSendToChat, onDelete }: {
  ...
  return (
    <div onClick={onOpen} className={isNew ? 'doc-card-new' : undefined} style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: spacing(3), cursor: 'pointer', display: 'flex', alignItems: 'flex-start', gap: spacing(2) }}>
      ...
      title={
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          ...
          <Tooltip title="Expand to full page">
            <ButtonGhost shape={buttonShapes.SQUARE} leftIcon={iconType.ArrowsExpandOutlined} onClick={onExpand} />
          </Tooltip>
        </div>
      }
    >
          ...
          <div style={{ flexShrink: 0, marginTop: -spacing(4), marginLeft: -spacing(6), marginRight: -spacing(6), paddingLeft: spacing(2), paddingRight: spacing(2), marginBottom: spacing(2) }}>
            <Toolbar
              ...
              leftItems={[
                ...
                <ButtonTertiary key="add-to-chat" leftIcon={iconType.PlusOutlined} onClick={() => onSendToChat(selectedDocs)}>
...
function DocumentsTableView({ space, docs, sourceMap, highlightDocIds, typeFilter, formatFilter, tagFilter, search, setSearch, selected, setSelected, page, setPage, onCollapse, onOpenFilter, onOpenAdd, onOpenDoc, onSendToChat, onDeleteDocs, onClearFilter, onClearAllFilters }: {
  space: Space
  docs: MetadataDocument[]
  sourceMap: Map<string, DocSource>
  highlightDocIds: Set<string>
  ...
  const columns = useMemo(() => {
      ...
      render: (_: unknown, record: MetadataDocument) => {
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            {sourceIcon(src, 16, src !== 'local' ? spaceConnectorLabel(space, src) : undefined)}
            <Typography size="base-sm" color="neutral-darken5">{sourceLabel(space, src)}</Typography>
          </div>
        )
      },
    }
    ...
    const formatCol = {
      ...
      width: 100,
      onCell: (record: MetadataDocument) => ({ onClick: () => onOpenDoc(record._id), style: rowStyle(record) }),
      render: (val: FileFormat) => <Chip label={val} chipStyle={chipStyles.ACCENT_NEUTRAL} variant={chipVariants.SUBTLE} />,
    }
    ...
    return [checkboxCol, nameCol, typeCol, tagsCol, sourceCol, uploadedCol, sizeCol, formatCol, actionsCol]
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', backgroundColor: colorPalette.white, overflow: 'hidden' }}>
      <div style={{ flexShrink: 0, padding: `${spacing(5)}px ${spacing(6)}px`, display: 'flex', flexDirection: 'column', gap: spacing(4), borderBottom: '1px solid #e5e7eb' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: spacing(2) }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: spacing(2) }}>
            <Tooltip title="Collapse to panel">
              <ButtonGhost shape={buttonShapes.SQUARE} leftIcon={iconType.ArrowsCompressOutlined} onClick={onCollapse} />
            </Tooltip>
            <Icon type={iconType.FolderOutlined} size={20} color="neutral-darken5" />
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', padding: `0 ${spacing(6)}px` }}>
        ...
        {filtered.length > DOCS_PAGE_SIZE && (
          <div style={{ flexShrink: 0, display: 'flex', justifyContent: 'center', padding: `${spacing(3)}px 0` }}>
            <Pagination current={page} total={filtered.length} pageSize={DOCS_PAGE_SIZE} onChange={setPage} />
          </div>
        )}
      </div>
...
function DocPreviewView({ space, doc, source, onBack, onSendToChat }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <div style={{ flexShrink: 0, height: 52, backgroundColor: '#1e1f2e', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: `0 ${spacing(4)}px` }}>
        ...
        <div style={{ display: 'flex', gap: spacing(2) }}>
          <ButtonTertiary mode="contrast" leftIcon={iconType.SendOutlined} onClick={onSendToChat}>Send to chat</ButtonTertiary>
          <Dropdown
            items={[
              { key: 'download', label: <span style={{ display: 'flex', alignItems: 'center', gap: spacing(2) }}><Icon type={iconType.DownloadOutlined} size={16} />Download</span>, onClick: () => {} },
            ...
            placement={dropdownPlacement.BOTTOM_RIGHT}
          >
            <ButtonTertiary mode="contrast" shape={buttonShapes.SQUARE} leftIcon={iconType.ThreeDotsHorOutlined} />
          </Dropdown>
        </div>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', padding: spacing(6), display: 'flex', gap: spacing(4), alignItems: 'flex-start', backgroundColor: '#f0f2f5' }}>
        <div style={{ flex: '0 0 60%', backgroundColor: colorPalette.white, borderRadius: 8, padding: `${spacing(8)}px ${spacing(10)}px`, minHeight: 640 }}>
          <MockDocumentBody doc={doc} />
        </div>

        <div style={{ flex: 1, minWidth: 0, backgroundColor: colorPalette.white, borderRadius: 8, padding: spacing(6) }}>
