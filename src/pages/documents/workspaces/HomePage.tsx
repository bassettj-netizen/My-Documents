...
// Front door into the three real products in the suite — Home's job is getting people into
// the right one fast, not duplicating what's already built inside each of them.
const QUICK_LAUNCH: { key: string; label: string; description: string; icon?: string; customIcon?: React.ReactNode; path: string }[] = [
  // CoPilot's own brand mark, not a generic DS icon — same mark used in its own page
  // header and the global sidebar, so Home doesn't invent a fourth stand-in for it.
  { key: 'copilot', label: 'CoPilot', description: 'Ask a tax question and get sourced answers', customIcon: <CoPilotMark size={20} />, path: `${LAUNCH_PAD_BASE}/copilot` },
    ...
    <div style={{ height: '100%', overflowY: 'auto' }}>
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: `${spacing(10)}px ${spacing(5)}px ${spacing(12)}px`, display: 'flex', flexDirection: 'column', gap: spacing(9) }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: spacing(5), alignItems: 'center', textAlign: 'center' }}>
          {/* Typography's size enum has no 32px step between its available options, so this
              bypasses it for the exact size, matching the plain-span pattern used elsewhere. */}
        ...
        {/* Quick launch into the three real products */}
