...
const LAUNCH_PAD_BASE = '/projects/workspaces/launch-pad'
const WORKSPACES_BASE = `${LAUNCH_PAD_BASE}/workspaces`

// Same custom mark used by the global AppShell sidebar (App.tsx) — kept as its own
// copy here since Launch Pad's shell is intentionally independent of AppShell's.
function CoPilotIcon() {
  return (
    <div className="goat-menu-item-icon" style={{ color: colorPalette.neutral.base }}>
      <svg width={24} height={24} viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        <mask id="copilot-launchpad-icon-mask" style={{ maskType: 'luminance' }} maskUnits="userSpaceOnUse" x="0" y="0" width="22" height="22">
          <path d="M22 0H0V22H22V0Z" fill="white" />
        </mask>
        <g mask="url(#copilot-launchpad-icon-mask)">
...
// https://www.svgrepo.com/svg/352587/tools — the sidebar's own Tools icon, deliberately a
// different (bolder, filled) mark from the outline one ToolsPage's own header uses next to
// its title — the two are no longer meant to be the same glyph.
...
// Stands in for every page in the new product-suite IA that isn't built yet —
// just the screen title, until each one gets designed and implemented for real.
...
function LaunchPadShell() {
  const navigate = useNavigate()
  const location = useLocation()
  const [collapsed, setCollapsed] = useState(true)

  // The avatar row is deliberately NOT a real bottomItems entry (see below), so it can't
  // just sit in the menu's own flow under Settings — its position has to be measured against
  // Settings' actual rendered box rather than guessed as a fixed pixel offset. A guess drifts
  // out of sync with wherever the sidebar library actually lays Settings out (confirmed: it
  // looked right in one browser/viewport and overlapped in another).
  ...
  useLayoutEffect(() => {
    const measure = () => {
      ...
      // Vertical: measure the enclosing menu row, not just the icon glyph — the icon sits
      // inset within the row (centered in a taller clickable/hoverable box), so anchoring off
      // the icon alone put the avatar a few px too high, just inside the row's own hover area.
      // Horizontal: measure the icon itself, not the row — when expanded the row spans the
      // full (now much wider) rail including the label, so its own center drifts well past
      // the icon column the avatar needs to line up under.
    ...
    // Collapsing/expanding reflows the whole rail (icon-only vs icon+label widths), so the
    // measured position needs a beat to catch the post-transition layout, not the mid-animation one.
  ...
  const sidebarTopItems = [
    ...
    },
    {
      ...
      // Renders the existing Workspaces app inline (see the `workspaces/*` route below) rather
      // than jumping out to its standalone route — it stays inside this shell's sidebar/nav.
  ...
  const sidebarBottomItems = [
    ...
    {
      // Inert placeholder — reserves the same second row Settings+Avatar used to occupy
      // together, so Settings keeps its original spacing above the trigger instead of
      // sliding down to hug it now that the avatar itself is no longer a real menu entry.
      // No label/icon/onClick, so it has nothing to show on hover; the floating avatar
      // below sits directly on top of it and intercepts hover before it ever reaches this.
      key: 'avatar-spacer',
    },
  ] as unknown as SidebarItem[]
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', position: 'relative' }}>
      {/* Sidebar used directly (not via Layout) so this shell has no page header —
          Layout always renders a Header region even with no `header` prop passed. */}
      <Sidebar
        collapsed={collapsed}
      ...
      {/* Plain floating row, not a real bottomItems entry — the DS Avatar's built-in hover
          mask (an edit-photo camera icon that always shows on hover, with no prop to turn it
          off) and the menu system's own hover tooltip combined to make this look broken/
          cropped. Position comes from measuring Settings' own rendered box (settingsIconRef)
          rather than a fixed pixel guess — a guess drifted out of sync with wherever the
          sidebar library actually laid Settings out between environments. z-index above the
          sidebar's own (6) — sibling markup with no z-index paints behind a `position: sticky`
          element, not on top of it the way plain DOM order would suggest. */}
      {avatarPos && (
