...
export default function MetadataUserTestingV5() {
  ...
  const columns = useMemo(() => {
    const visible: object[] = FIXED_COLS.map(({ key, label }) => {
      const isNonEditable = NON_EDITABLE_KEYS.has(key) || key === 'tags' || key === 'name'
      ...
      const col: Record<string, unknown> = {
        ...
        onCell: (record: MetadataDocument) => ({
          onClick: (editingKey === record._id || record._id.startsWith('temp-')) ? undefined : () => navigate(`/my-documents/metadata/version-5/${record._id}`),
    ...
    return visible
  }, [navigate, editingKey, editingTags, tagOptions, handleCellChange, startEdit, saveEdit, cancelEdit])
                ...
                {searchResults.map(({ doc, relevantMentions, excerpt }, idx) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        navigate(`/my-documents/metadata/version-5/${doc._id}`)
                        setShowDropdown(false)
                      }}
                    >
