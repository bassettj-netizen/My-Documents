...
export default function MetadataVersion7() {
  ...
  const [panelDoc, setPanelDoc] = useState<MetadataDocument | null>(null)
  ...
  const openPanel = useCallback((record: MetadataDocument) => {
    setPanelDoc(record)
    setEditingDocumentType(record.documentType)
  ...
  const savePanel = useCallback(() => {
    ...
    const updated: MetadataDocument = {
      ...
      tagList: editingTags.map(t => ({ text: t, style: chipStyles.ACCENT_NEUTRAL, variant: chipVariants.HIGHLIGHT })),
      namedEntity: '—',
      namedEntityId: '—',
      jurisdiction: '—',
      lawType: '—',
  ...
  }, [panelDoc, editingDocumentType, editingTags, handleSave, closePanel, notification])
    ...
    const visible: object[] = FIXED_COLS.map(({ key, label }) => {
      ...
      const col: Record<string, unknown> = {
        ...
        onCell: (record: MetadataDocument) => ({
          onClick: () => navigate(`/my-documents/metadata/version-7/${record._id}`),
      ...
      render: (_: unknown, record: MetadataDocument) => (
        <Dropdown
          items={[
            ...
            { key: 'edit', label: <span style={{ display: 'flex', alignItems: 'center', gap: spacing(2) }}><Icon type={iconType.EditRecOutlined} size={16} />Edit document info</span>, onClick: () => openPanel(record) },
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, backgroundColor: colorPalette.white, height: '100%', overflow: 'hidden' }}>
      ...
      <Panel
        ...
        size={panelSizes.CUSTOM}
        width={480}
        overlay
        overlayVariant={panelOverlayVariants.DEFAULT}
        onClose={closePanel}
        footer={{
          buttons: [
            { variant: buttonVariants.TERTIARY, props: { children: 'Cancel', onClick: closePanel } },
            { variant: buttonVariants.PRIMARY, props: { children: 'Save', onClick: savePanel } },
          ],
        }}
      >
        {panelDoc && (
