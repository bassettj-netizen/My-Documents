...
function InlineTagInput({
  ...
  return (
    <div ref={containerRef} onClick={e => e.stopPropagation()}>
      {/* Tag chips + text input in one row */}
      <div
        style={{
      ...
      >
        ...
        <input
          ...
          style={{
            border: 'none', outline: 'none', flex: '1 1 80px', minWidth: 80,
            fontSize: 13, padding: '2px 2px', backgroundColor: 'transparent',
            fontFamily: 'inherit',
          }}
        />
      </div>
...
export default function MetadataUserTestingV7() {
  ...
  const columns = useMemo(() => {
    ...
    return visible
  }, [navigate, editingCell, editingTags, cellValue, handleSave])
    ...
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, backgroundColor: colorPalette.white, height: '100%', overflow: 'hidden' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        ...
        >
          ...
          {showDropdown && searchResults.length > 0 && (
            <div style={{
              position: 'absolute', top: 'calc(100% + 4px)', right: 0, width: 400,
              backgroundColor: colorPalette.white, border: '1px solid #e5e7eb',
              borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.12)', zIndex: 200, overflow: 'hidden',
            }}>
      ...
      <div style={{ flexShrink: 0, width: '100%' }}>
        ...
        <FileUploader onUpload={handleUpload} accept={['.pdf', '.docx', '.xlsx', '.txt']} {...({ multiple: true } as any)}>
          <div
            ...
            style={{
              border: '1.5px dashed #d0d0d0', borderRadius: 8, padding: '28px 24px',
              display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
              cursor: isUploading ? 'not-allowed' : 'pointer', width: '100%',
              backgroundColor: isUploading ? '#F0F0F0' : undefined,
              pointerEvents: isUploading ? 'none' : 'auto', transition: 'background-color 0.2s ease',
            }}
          >
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
        {isInitialLoading ? (
          ...
          <Table dataSource={pagedDocs} columns={columns as never} pagination={false} innerLoading={isUploading || isSearching} />
        )}
      </div>
