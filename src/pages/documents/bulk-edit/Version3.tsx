...
export default function BulkEditV3() {
  ...
  // True when a row is being edited or at least one row is selected
  const isInteractionLocked = editingKey !== null || selectedKeys.size > 0
  ...
  const columns = useMemo(() => {
    ...
    const visible: object[] = FIXED_COLS.map(({ key, label }) => {
      ...
      const col: Record<string, unknown> = {
        ...
        onCell: (record: MetadataDocument) => ({
          onClick: (editingKey === record._id || record._id.startsWith('temp-')) ? undefined : () => navigate(`/my-documents/bulk-edit/version-3/${record._id}`),
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, backgroundColor: colorPalette.white, height: '100%', overflow: 'hidden' }}>
                ...
                {searchResults.map(({ doc, relevantMentions, excerpt }, idx) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        navigate(`/my-documents/bulk-edit/version-3/${doc._id}`)
                        setShowDropdown(false)
                      }}
                    >
      ...
      <div style={{ flexShrink: 0, width: '100%', position: 'relative' }}>
        {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
        <FileUploader onUpload={handleUpload} accept={['.pdf', '.docx', '.xlsx', '.pptx']} {...({ multiple: true } as any)}>
          ...
          >
            <Icon type={iconType.UploadOutlined} color={(isUploading || isInteractionLocked) ? 'disabled-lighten1' : 'neutral-darken4'} />
            <Typography color={(isUploading || isInteractionLocked) ? 'disabled-lighten1' : 'neutral-darken5'}>Click to select a document or drag it here.</Typography>
            <Typography size="base-sm" color={(isUploading || isInteractionLocked) ? 'disabled-lighten1' : 'neutral-darken2'}>PDF, DOCX, XLSX and PPTX formats, max. 10 MB</Typography>
          </div>
        </FileUploader>
