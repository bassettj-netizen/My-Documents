...
type DocSource = ConnectorType | 'local'
...
function sourceIcon(source: DocSource, size = 20) {
  if (source === 'local') return <Icon type={iconType.ShieldFilled} size={size as never} color="primary-base" />
  return connectorIcon(source, size)
}
...
function getLastUpload(docs: MetadataDocument[]) {
...
function SpaceAvatar({ space, size = 36 }: { space: Space; size?: number }) {
  ...
  return (
    <Modal
      ...
      footer={{ buttons: [
        ...
        { variant: buttonVariants.PRIMARY, props: { children: 'Create space', onClick: () => onCreate(name.trim(), description.trim()), disabled: !name.trim() } },
      ]}}
    >
...
function SpaceCard({ space, docCount, lastUpload, onClick, onRequestDelete }: {
  space: Space
  docCount: number
  lastUpload: string
  onClick: () => void
  ...
  const isMyDocs = space.id === MY_DOCS_SPACE_ID
  const primaryConnector = space.connectors[0]
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        {isMyDocs ? (
          ...
          <div onClick={e => e.stopPropagation()}>
            <Dropdown
              items={[{ key: 'delete', label: <span style={{ color: colorPalette.danger.darken2 }}>Delete space</span>, onClick: () => onRequestDelete(space.id) }]}
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {!isMyDocs && <Icon type={iconType.CompanyOutlined} size={16} color="neutral-darken2" />}
        </div>
      ...
      {!isMyDocs && primaryConnector && (
        <div style={{ position: 'absolute', bottom: spacing(4), right: spacing(4) }}>
          {connectorIcon(primaryConnector.type, 20)}
        </div>
      )}
    </div>
  )
...
function SpaceDetail({ space, docs, sidebarWidth, onBack }: {
  space: Space
  ...
  // Deterministic per-document source, derived from the space's connectors.
  const docSourceMap = useMemo(() => {
    const map = new Map<string, DocSource>()
    localDocs.forEach((d, i) => {
      map.set(d._id, space.connectors.length === 0 ? 'local' : space.connectors[i % space.connectors.length].type)
    })
    return map
  }, [localDocs, space.connectors])
  ...
  const columns = useMemo(() => {
    ...
    const visible: object[] = FIXED_COLS.map(({ key, label }) => {
      ...
      if (key === 'source') {
        col.width = 64
        col.render = (_: unknown, record: MetadataDocument) => sourceIcon(docSourceMap.get(record._id) ?? 'local', 20)
      }
  ...
  }, [editingKey, selectedKeys, handleCellChange, startEdit, saveEdit, cancelEdit, filteredDocs, allSelected, someSelected, docSourceMap])
    ...
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, backgroundColor: colorPalette.white, height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', gap: spacing(1) }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div ref={searchBarWrapperRef} style={{ position: 'relative', width: 300 }}>
            <SearchBar
...
export default function SharepointVersion7() {
  ...
  const handleCreateSpace = (name: string, description: string) => {
    const newSpace: Space = { id: `space-${Date.now()}`, name, description, connectors: [] }
  ...
  const handleDeleteSpace = () => {
    ...
    setSpaces(prev => prev.filter(s => s.id !== deleteTargetSpace.id))
    notification.default({ title: `"${deleteTargetSpace.name}" deleted`, placement: toastPlacements.BOTTOM_LEFT, duration: 3 })
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: spacing(6), backgroundColor: colorPalette.white, minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <ButtonSecondary leftIcon={iconType.PlusOutlined} onClick={() => setCreateModalOpen(true)}>Space</ButtonSecondary>
      </div>
      ...
      <Modal
        ...
        footer={{ buttons: [
          ...
          { variant: buttonVariants.DANGER, props: { children: 'Delete', onClick: handleDeleteSpace } },
        ]}}
      >
        <Typography size="base" color="neutral-darken5">
