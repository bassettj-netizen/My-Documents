...
// Deterministic connector assignment for mock docs that have no embedded connector info
function deterministicConnectorFor(docId: string, connectors: Connector[]): Connector | null {
  if (!connectors.length) return null
  // Extra slot so not every doc gets a connector (~1/n+1 docs show none)
  const buckets = connectors.length + 1
  let hash = 0
  for (let i = 0; i < docId.length; i++) hash = (hash * 31 + docId.charCodeAt(i)) | 0
  const idx = Math.abs(hash) % buckets
  return idx < connectors.length ? connectors[idx] : null
}
  ...
  return (
    <div ref={ref} style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
      {tags.slice(0, visibleCount).map((t, i) => (
        <Chip key={i} label={t.text} chipStyle={t.style as typeof chipStyles[keyof typeof chipStyles]} variant={(t.variant as typeof chipVariants[keyof typeof chipVariants]) ?? chipVariants.SUBTLE} />
      ))}
      {hidden.length > 0 && (
...
function ConnectorStatusDot({ connected }: { connected: boolean }) {
  return (
    <div style={{
      width: 7,
      height: 7,
...
function CompactConnectorStrip({ connectors, onSync, onDisconnect, onAdd, canAdd }: {
  connectors: Connector[]
  ...
  return (
    <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: spacing(2), flexWrap: 'wrap' }}>
      {connectors.map(c => (
        <div
          key={c.id}
          style={{
        ...
        >
          {connectorIcon(c.type, 14)}
          <ConnectorStatusDot connected />
          <Typography size="base-sm" color="neutral-darken5" weight={fontWeight.SEMIBOLD}>
          ...
          <Dropdown
            items={[
              ...
              { key: 'disconnect', label: <span style={{ color: colorPalette.danger.darken2 }}>Disconnect</span>, onClick: () => onDisconnect(c.id) },
            ]}
      ...
      {connectors.length === 0 && (
        <Typography size="base-sm" color="neutral-darken2">No integrations connected.</Typography>
      )}
      {canAdd && (
        <ButtonGhost leftIcon={iconType.PlusOutlined} onClick={onAdd}>Add integration</ButtonGhost>
      )}
    </div>
  )
}
...
function SpaceDetail({ space, docs, sidebarWidth, onBack, onAddConnector, onConnectorSync, onConnectorDisconnect }: {
    ...
    const visible: object[] = FIXED_COLS.map(({ key, label }) => {
      ...
      if (key === 'name') {
        ...
        col.render = (name: string, record: MetadataDocument) => {
          const isLocalUpload = record._id.startsWith('upload-')

          let sourceConnector: Connector | null = null
          if (!isMyDocs) {
            if (record._id.startsWith('int-')) {
              // Integration import: find matching connector by type prefix in key
              const keyPart = record._id.replace(/^int-/, '').replace(/-\d+$/, '')
              const prefix = keyPart.split('-')[0]
              sourceConnector = space.connectors.find(c => c.type.startsWith(prefix) || c.type === prefix) ?? null
            } else if (!isLocalUpload) {
              // Mock doc: assign connector deterministically by doc ID hash
              sourceConnector = deterministicConnectorFor(record._id, space.connectors)
            }
          }
          ...
          return (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, overflow: 'hidden' }}>
              ...
              {sourceConnector && (
                <Tooltip title={`${connectorLabel(sourceConnector.type)}: ${sourceConnector.label}`}>
                  <div style={{ flexShrink: 0, opacity: 0.65, display: 'flex', alignItems: 'center' }}>
                    {connectorIcon(sourceConnector.type, 14)}
                  </div>
                </Tooltip>
              )}
              {isLocalUpload && (
  ...
  }, [editingKey, selectedKeys, handleCellChange, startEdit, saveEdit, cancelEdit, filteredDocs, allSelected, someSelected, space.connectors, isMyDocs])
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, backgroundColor: colorPalette.white, height: '100%', overflow: 'hidden' }}>
      ...
      {/* Compact connector strip — not shown for My Documents */}
      {!isMyDocs && (
        <div style={{ flexShrink: 0 }}>
          <CompactConnectorStrip
            connectors={space.connectors}
            onSync={onConnectorSync}
            onDisconnect={onConnectorDisconnect}
            onAdd={onAddConnector}
            canAdd={space.connectors.length < 4}
          />
        </div>
      )}

      {/* Document toolbar */}
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid #f0f0f0', paddingTop: spacing(3) }}>
        <Typography size="base-sm" color="neutral-darken2">
...
export default function SharepointVersion5() {
