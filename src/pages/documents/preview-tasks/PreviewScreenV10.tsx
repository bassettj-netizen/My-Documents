...
type AiPopupPhase = 'instructions' | 'generating' | 'result'
type AiPopupSource = 'selection' | 'document'
interface AiEditResult { original: string; suggested: string }
...
export default function PreviewTasksPreviewScreenV10() {
  ...
  useEffect(() => {
    ...
    const cancel = (e: MouseEvent) => {
      if (!docBodyRef.current?.contains(e.target as Node) && !selToolbarRef.current?.contains(e.target as Node) && !editToolbarRef.current?.contains(e.target as Node)) {
  ...
  useEffect(() => {
    const onMouseUp = (e: MouseEvent) => {
      ...
      walk(frag)
      setTextSelection({ text: selectedText, x: Math.round(rect.left + rect.width / 2), y: Math.round(rect.top), bottom: Math.round(rect.bottom) })
    }
  ...
  useLayoutEffect(() => {
    ...
    const margin = 8; const vw = window.innerWidth
    if (rect.left < margin) setToolbarAdjustX(prev => prev + (margin - rect.left))
    else if (rect.right > vw - margin) setToolbarAdjustX(prev => prev - (rect.right - (vw - margin)))
    else setToolbarAdjustX(0)
  ...
  const openAiPopupFromTask = () => {
    const docRect = docBodyRef.current?.getBoundingClientRect()
    if (!docRect) return
    const left = Math.min(docRect.left + 24, docRect.right - POPUP_W - 8)
    const top = Math.min(docRect.top + 72, window.innerHeight - 420)
    setAiPopupPos({ top, left })
  ...
  const submitAiEdit = () => {
    if (!aiPopupInstruction.trim()) return
    const docText = docContentRef.current?.innerText?.trim() || `${displayDoc.documentType} — ${displayDoc.namedEntity}`
    const original = aiPopupSource === 'selection' ? aiPopupSelectedText : docText
    setAiPopupPhase('generating')
    setTimeout(() => {
      setAiPopupResult({ original, suggested: simulateGptEdit(original, aiPopupInstruction) })
      setAiPopupPhase('result')
    }, 1600)
  }
  ...
  const selectionAction = (action: string) => {
    if (action === 'ai-edit') { openAiPopupFromSelection(); return }
    ...
    notification.default({ title: labels[action] ?? action, content: <Typography size="base" color="neutral-darken5">"{short}"</Typography>, placement: toastPlacements.BOTTOM_RIGHT, duration: 4 })
    setTextSelection(null)
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#F5F9FF' }}>
      ...
      {textSelection && (
        ...
        >
          {isDocEditing && (
            <div style={{ backgroundColor: colorPalette.white, borderRadius: 8, padding: '3px 6px', display: 'flex', alignItems: 'center', gap: 2, boxShadow: '0 4px 16px rgba(0,0,0,0.14)', border: '1px solid #e5e7eb' }}>
          ...
          <div style={{ backgroundColor: colorPalette.white, borderRadius: 8, padding: '3px 6px', display: 'flex', alignItems: 'center', gap: 2, boxShadow: '0 4px 16px rgba(0,0,0,0.14)', border: '1px solid #e5e7eb' }}>
            {!isDocEditing && (
              <>
                <ButtonGhost leftIcon={iconType.EditOutlined} onMouseDown={() => { const sel = window.getSelection(); if (sel && sel.rangeCount > 0) savedSelRangeRef.current = sel.getRangeAt(0).cloneRange() }} onClick={enterDocEditFromSelection}>Edit</ButtonGhost>
                <div style={{ width: 1, height: 16, backgroundColor: '#e5e7eb', margin: '0 2px', flexShrink: 0 }} />
              </>
            )}
      ...
      {aiPopupOpen && (
        <AiEditPopup
          ...
          onQuickAction={v => setAiPopupInstruction(v)}
          onSubmit={submitAiEdit}
          onDiscard={() => { setAiPopupPhase('instructions'); setAiPopupResult(null) }}
          onApply={applyAiEdit}
...
const AiEditPopup = React.forwardRef<HTMLDivElement, {
  ...
  onApply: () => void
  onClose: () => void
}>(function AiEditPopup({ phase, source, selectedText, instruction, result, pos, onInstructionChange, onQuickAction, onSubmit, onDiscard, onApply, onClose }, ref) {
  const contextSnippet = selectedText.length > 60 ? selectedText.slice(0, 60) + '…' : selectedText
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: `${spacing(3)}px ${spacing(4)}px`, borderBottom: '1px solid #f0f0f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: spacing(2) }}>
          <Typography size="base" weight="semibold" color="neutral-darken5">
            {isResult ? 'Suggested changes' : 'Edit with AI'}
          </Typography>
          {!isResult && (
            <Chip
              label={source === 'selection' ? 'Selection' : 'Entire document'}
        ...
        {!isResult && (
          <>
            ...
            {source === 'selection' && selectedText && (
              <div style={{ backgroundColor: '#F8FAFF', borderRadius: 4, padding: `${spacing(2)}px ${spacing(3)}px`, borderLeft: `3px solid ${colorPalette.blue.base}` }}>
                <Typography size="base-sm" color="neutral-darken3">"{contextSnippet}"</Typography>
              </div>
            )}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: spacing(2) }}>
              ...
              >
                {isGenerating
                  ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}><Spinner size="small" />Generating…</span>
                  : 'Submit'
                }
              </ButtonPrimary>
            </div>
        ...
        {isResult && result && (
          <>
            <div style={{ display: 'flex', flexDirection: 'column', gap: spacing(2) }}>
              ...
              <div style={{ backgroundColor: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 6, padding: spacing(3), maxHeight: 120, overflowY: 'auto', fontSize: 12, lineHeight: '1.6', color: colorPalette.neutral.darken5, whiteSpace: 'pre-wrap' }}>
                {result.original}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: spacing(2) }}>
              ...
              <div style={{ backgroundColor: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 6, padding: spacing(3), maxHeight: 120, overflowY: 'auto', fontSize: 12, lineHeight: '1.6', color: colorPalette.neutral.darken5, whiteSpace: 'pre-wrap' }}>
                {result.suggested}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: spacing(2) }}>
              ...
              <ButtonPrimary leftIcon={iconType.EditRecOutlined} onClick={onApply}>Apply changes</ButtonPrimary>
            </div>
          </>
        )}
      </div>
...
function TasksPanel({ taskStates, onRun, onAiEdit }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {/* Edit with AI card */}
      <div
        style={{ border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden', cursor: 'pointer', transition: 'border-color 0.15s', backgroundColor: colorPalette.white }}
        onClick={onAiEdit}
        onMouseEnter={e => { e.currentTarget.style.borderColor = '#a5b4fc' }}
        onMouseLeave={e => { e.currentTarget.style.borderColor = '#e5e7eb' }}
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px' }}>
          <span style={{ flexShrink: 0, marginTop: 2, display: 'inline-flex', color: colorPalette.neutral.darken4 }}>
            <SparkleIcon size={16} />
          </span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <Typography size="base" weight="semibold" color="neutral-darken5">Edit with AI</Typography>
            <Typography size="base-sm" color="neutral-darken2">Rewrite or improve document content using AI.</Typography>
          </div>
        </div>
      </div>
    </div>
