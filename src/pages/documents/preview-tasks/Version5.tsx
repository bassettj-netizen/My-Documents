  ...
  const columns = useMemo(() => {
    ...
    const visible: object[] = FIXED_COLS.map(({ key, label }) => {
      ...
      const col: Record<string, unknown> = {
        ...
        onCell: (record: MetadataDocument) => ({
          onClick: (editingKey === record._id || record._id.startsWith('temp-')) ? undefined : () => navigate(`/my-documents/preview-tasks/version-5/${record._id}`),
                ...
                {searchResults.map(({ doc, relevantMentions, excerpt }, idx) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        navigate(`/my-documents/preview-tasks/version-5/${doc._id}`)
                        setShowDropdown(false)
                      }}
                    >
