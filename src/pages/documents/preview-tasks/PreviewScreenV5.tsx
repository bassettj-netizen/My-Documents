      ...
      return (
        <div style={gap8}>
          ...
          {related.map(r => (
            <div
              key={r._id}
              onClick={() => window.open(`/my-documents/preview-tasks/version-5/${r._id}`, '_blank', 'noopener,noreferrer')}
...
export default function PreviewTasksPreviewScreenV5() {
  ...
  // Document body edit state — triggered by the Edit button, not by clicking the document
  ...
  const savedDocHtmlRef = useRef<string>('')
  ...
  useEffect(() => {
    ...
    setDocHtml(null); setIsDocEditing(false); setHasDocChanges(false)
    docHtmlInitialized.current = false
    savedDocHtmlRef.current = ''
  ...
  // Place cursor at start of first text node when entering edit mode
  useEffect(() => {
  ...
  // Measure document card bounds so the fixed toolbar can match its width and position
  useEffect(() => {
  ...
  // If no changes have been made, clicking outside the document exits edit mode silently
  useEffect(() => {
  ...
  if (!foundDoc) return <Navigate to="/my-documents/preview-tasks/version-5" replace />
  ...
  const discardDocEdit = () => {
    // docHtml state equals savedDocHtmlRef at this point (it never mutates during editing),
    // so React won't re-render the innerHTML — write it directly to the DOM instead.
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#F5F9FF' }}>
      ...
      <div style={{ flex: 1, padding: 24, display: 'flex', gap: 16, alignItems: 'flex-start' }}>
        ...
        >
          {/* Edit button — top-right of document, only visible when not loading or editing */}
          ...
          {isLoading
            ...
            // reconciles user-typed content away during re-renders.
            ? <div
                ref={docContentRef}
      ...
      {/* Text selection toolbar — visible regardless of document edit mode */}
      {textSelection && (
        <div
          ref={selToolbarRef}
          style={{
      ...
      {/* Save / Discard toolbar — fixed to bottom, aligned to document card width, only shown after a change */}
      {isDocEditing && hasDocChanges && docToolbarStyle && (
        <div style={{ position: 'fixed', bottom: spacing(2), left: docToolbarStyle.left + spacing(2), width: docToolbarStyle.width - spacing(2) * 2, zIndex: 200 }}>
          <Toolbar
            ...
            rightItems={[
              <ButtonTertiary key="discard" onClick={discardDocEdit}>Discard</ButtonTertiary>,
              <ButtonPrimary key="save" onClick={saveDocEdit}>Save</ButtonPrimary>,
            ]}
          />
        </div>
      )}
