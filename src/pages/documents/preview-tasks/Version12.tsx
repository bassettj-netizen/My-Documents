  ...
  const columns = useMemo(() => {
    ...
    const visible: object[] = FIXED_COLS.map(({ key, label }) => {
      ...
      const col: Record<string, unknown> = {
        ...
        onCell: (record: MetadataDocument) => ({
          onClick: (editingKey === record._id || record._id.startsWith('temp-')) ? undefined : () => navigate(`/projects/preview-tasks/version-12/${record._id}`),
                ...
                {searchResults.map(({ doc, relevantMentions, excerpt }, idx) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        navigate(`/projects/preview-tasks/version-12/${doc._id}`)
                        setShowDropdown(false)
                      }}
                    >
