      ...
      return (
        <div style={gap8}>
          ...
          {related.map(r => (
            <div
              key={r._id}
              onClick={() => window.open(`/my-documents/preview-tasks/version-6/${r._id}`, '_blank', 'noopener,noreferrer')}
...
export default function PreviewTasksPreviewScreenV6() {
  ...
  if (!foundDoc) return <Navigate to="/my-documents/preview-tasks/version-6" replace />
  ...
  const stackedToolbarHeight = isDocEditing ? 88 : 40
  const toolbarAboveY = (textSelection?.y ?? 0) - stackedToolbarHeight - 8
  const showToolbarBelow = toolbarAboveY < 60
  const toolbarY = showToolbarBelow ? (textSelection?.bottom ?? 0) + 8 : toolbarAboveY
