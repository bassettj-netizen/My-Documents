    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#f0f2f5' }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 100, backgroundColor: TOP_BAR_BG, padding: '0 24px', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <ButtonTertiary mode="contrast" rightIcon={iconType.ExternalLinkOutlined} onClick={openPanel}>
            Ask CoPilot
          </ButtonTertiary>
      ...
      <div style={{ backgroundColor: colorPalette.white, borderBottom: '1px solid #f0f0f0', padding: '10px 24px', display: 'flex', gap: 32, alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <Typography size="base-sm" color="neutral-darken2">NAME</Typography>
          <Typography size="base-sm" color="neutral-darken5">{doc.name}</Typography>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <Typography size="base-sm" color="neutral-darken2">UPLOADED</Typography>
          <Typography size="base-sm" color="neutral-darken5">{doc.uploadedAt}</Typography>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <Typography size="base-sm" color="neutral-darken2">FORMAT</Typography>
          <Chip label={doc.format} chipStyle={chipStyles.ACCENT_NEUTRAL} variant={chipVariants.SUBTLE} />
        </div>
      ...
      <div style={{ flex: 1, padding: 24 }}>
        <div style={{ backgroundColor: colorPalette.white, borderRadius: 8, padding: '32px 40px', maxWidth: 860, minHeight: 640 }}>
          {isLoading
      ...
      {/* Sliding CoPilot panel */}
      <Panel
        visible={panelOpen}
        title={<Typography weight="bold">CoPilot</Typography>}
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, height: '100%' }}>
          ...
          <div style={{ borderTop: '1px solid #f0f0f0', paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <PropertyItem label="DOCUMENT" value={doc.name} labelProps={{ size: 'base-sm', color: 'neutral-darken2' }} />
